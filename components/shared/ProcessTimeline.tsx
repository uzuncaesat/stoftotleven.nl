  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="absolute left-[1.35rem] top-3 hidden h-[calc(100%-1.5rem)] w-px origin-top bg-terracotta md:left-0 md:top-[1.6rem] md:h-px md:w-full md:origin-left"
      />
      ...
      {steps.map((s, i) => (
        ...
        >
          <span className="absolute left-0 top-0 flex h-11 w-11 items-center justify-center rounded-full border border-terracotta/50 bg-linen font-display text-lg text-terracotta md:-top-[1.4rem] md:left-0">
            {String(i + 1).padStart(2, "0")}
          </span>
          <h3 className="font-display text-[1.5rem] leading-tight text-charcoal">{s.title}</h3>
